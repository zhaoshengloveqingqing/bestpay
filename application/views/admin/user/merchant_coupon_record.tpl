	...
	{block name="main"}
		{container}
			...
			{row layout=[6,12] layout-lg=[12]}					
				{form name="admin/merchant_coupon_record" method="get"}
					{row layout=[12] layout-lg=[12]}
						{field field="group_purchase" label-class="col-xs-12 col-lg-2" input-class="col-xs-12  col-lg-8"}
	                      					{select options=$products label-field="name" value-field="id"}{/select}
						{/field}	
					{/row}
					{row clsss="time" layout=[12,12] layout-lg=[4,3,1,1,1]}
			...
			{/row}
			{row}
				{datatable name="admin_merchant_coupon_record"}
			{/row}
		{/container}
	{/block}
	{block name="foot"}		
