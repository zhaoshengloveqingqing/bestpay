									...
									{tbody}
										{tr}
											{for $foo=1 to 1}
												{td}
													{div}
														{form name="admin/merchant/order_distribute"}
															{field label-class="col-lg-2" input-class="col-lg-4" field="merchant_name"}
																{select options=$children_merchants}
																{/select}
															{/field}
															{submit value="Distribution" id="Distribution"}
															<input type="text" id="order_id" name="order_id" style="display:none" value="{$orders->id}">
														{/form}
													{/div}
												{/td}
											{/for}
			...
			{*{div class="panel"}*}
				...
				{*{div}*}
					{*{form name="admin/merchant/order_distribute"}*}
						...
						{*{field label-class="col-xs-2 col-lg-2" input-class="col-xs-10 col-lg-10" field="merchant_name"}*}
						{*{select*}
						{*options=$children_merchants}*}
						{*{/select}*}
					{*{/field}*}
						{*{row class="action"}*}
						{*{submit value="Distribution"}*}
						{*{/row}*}
					{*{/form}*}
				{*{/div}*}
			{*{/div}*}
